    ...
    <div className="fixed inset-0 z-[150] w-screen h-screen min-h-screen glass-overlay flex items-center justify-center p-4">
      <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-2xl space-y-6">
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-400">Masukkan tautan atau ID folder Google Drive yang ingin ditambahkan</p>
            </div>
          </div>
          <button onClick={onClose} className="text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white p-1 cursor-pointer">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Nama Label Akun</label>
            <input 
              type="text" 
              value={name} 
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Alamat Email (Opsional)</label>
            <input 
              type="email" 
              value={email} 
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">Tautan (URL) atau ID Folder Google Drive Publik</label>
            <input 
              type="text" 
              value={folderId} 
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200 dark:border-slate-800">
            <button 
              type="button" 
              onClick={onClose} 
              className="px-4 py-2 rounded-xl bg-slate-300 dark:bg-slate-800 hover:bg-slate-400 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-300 font-bold text-xs transition-all cursor-pointer"
            >
              Batal
            </button>
            <button 
